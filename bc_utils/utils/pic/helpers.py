...
r = 19/2 #cm https://www.hamamatsu.com/us/en/product/type/R5912/index.html
...
def I_calc(x,l):
  ...
  if not np.isscalar(x):
    ...
    return exp(-x/l)
def N_calc(x,efficiency,length,dEdx,l):
...
def err_calc(true,pred):
  if isinstance(true,float) and isinstance(pred,float):
    if true < 1e-10:
