...
def plot_background_cuts(cuts_dfs,legend_labels,columns,title,scale='linear',ax=None):
  ...
  for i,mean in enumerate(means):
    ax.errorbar(columns,mean,yerr=errs[i],label=legend_labels[i],linewidth=3)
    ax.fill_between(columns,mean-errs[i],mean+errs[i],alpha=0.2)
...
def plot_efficiency_purity(cuts_dfs,columns,title,ax=None,event=0):
  """Plots cuts made and events at each stage, made in cuts script
  cuts_dfs: signal first, backgrounds next list of dataframes
  ...
  ax.plot(columns,purity,label='Purity',linewidth=3)
  ax.plot(columns,efficiency,label='Efficiency',linewidth=3)
  plt.xticks(columns,[labels[i] for i in columns])
  plt.setp(ax.get_xticklabels(), fontsize=xls,rotation=40)
  plt.setp(ax.get_yticklabels(), fontsize=xls)
...
def back_sig_hist(dfs,labels,key,precut_events,title=None,xlabel=None,ylabel='Count',**kwarg):
  ...
  ax = fig.add_subplot()
  ax.hist(xs,stacked=True,label=label_counts,**kwarg)
