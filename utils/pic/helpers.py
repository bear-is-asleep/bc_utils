...
def d_calc_lazy(x,coating,f=pi*r_pmt**2/A_cm,R=Ref_cov,E_ws=E_ws):
  ...
  if coating == 0: #COATED!!
    Rs = 1/(1-R) #This will give us the probability of being observed for n reflections, n being the dimension of Rs minus 1
  elif coating == 1:
    Rs = 1/(1-R*E_ws) #Combine uncoated with probability of photon being shifted
  #BTW we're using 1/1-x = sum _n=1 x^n 

  if np.isscalar(x): #Calculating dispersion for scalar
    return f*(1+Rs) #We're going to try this for now
  else: #Calculating dispersion for vector x input, ds are dispersion values
    return np.full(x.shape,f*(1+Rs)) #Try this for now
...
def N_calc_lazy(x,efficiency,length,dEdx,l,coating):
  #Photons that reach PMT as a function of x (distance from PMT)
  #Use lazy d method
  return N_0(length,dEdx)*d_calc_lazy(x,coating)*I_calc(x,l)*efficiency
...
def make_ref_df(pmts,muontracks,op_hits,events,subruns,runs,cwd,move_dfs=True):
  ...
  columns = ['ophit_ch','ophit_pred','ophit_obs','pred_err','run','subrun',
          ...
          'ophit_opdet_z','opdet_tpc','muontrk_tpc','muontrk_x1', 'muontrk_y1', 'muontrk_z1', 'muontrk_x2',
          'muontrk_y2', 'muontrk_z2','mhits','mtype','mean_distance','muontrk_dx','muontrk_length','ophit_pred_noatt',
          'pred_err_noatt','ophit_pred_nodisp','pred_err_nodisp','Reflectivity'] 
      ...
      for event in events:
        ...
        for i_muon,row_muon in muon_temp.iterrows():
          ...
          for i_op,row_op in pmts.iterrows():
            ...
            if reflectivity == 0:
              continue #skip events with no possible photons
            ...
            #No reflections
            if : #Only keep channel with max reflectivity (1 for no ref.)
  ...
  pmt_hits_df = pd.DataFrame(pmt_info,columns=columns)
  pmt_hits_df = pmt_hits_df.set_index(['run','subrun','event'])
  pmt_hits_df_trunc = truncate_df(pmt_hits_df,keys_to_sum={'ophit_pred','ophit_obs'},mod_chs=True) #mod channels
